...
\sourcefig{infographics/ch08-data-ink.jpg}{Tufte's Data-Ink Ratio: maximise the ink that encodes data, eliminate the ink that decorates. Every gridline, border, and legend that survives must earn its place.}
...
\sourcefig{infographics/ch08-gestalt.jpg}{Gestalt principles at work: proximity groups, similarity relates, closure implies, figure-ground separates. The eye organises before the mind interprets.}
...
\subsection{The Anatomy of a Perfect Slide: Title + Visual}

Every slide in your presentation should function as a billboard: if someone saw only that slide, they should grasp the main point in under five seconds. To achieve this, every slide needs exactly two components in harmony:\endnote{Anlak, \emph{StoryMakers: El Arte de Construir Narrativas que Venden Ideas}, 2026.}

\begin{enumerate}
  \item \textbf{The Action Title:} Not a topic (``Q3 Sales'') but a conclusion (``Q3 sales grew 15\%, driven by our new marketing campaign''). The title \emph{is} the message. It is the top of a mini-pyramid for that individual slide.
  \item \textbf{The Supporting Visual:} The chart, image, or diagram that \emph{proves} the title is true. If the title says sales grew 15\%, the chart must show that 15\% clearly. The visual's only purpose is to demonstrate that the headline is correct.
\end{enumerate}

\begin{alertbox}[THE TOPIC TITLE TRAP]
When your titles are topics (``Revenue Overview,'' ``Market Analysis''), you force the audience to do the analytical work: they must look at the data, interpret it, and arrive at their own conclusion --- which may not be the one you intended. When your titles are action statements, you do the work for them: you tell them what to conclude, and the visual confirms it. This is the Pyramid Principle applied at slide level.
\end{alertbox}
