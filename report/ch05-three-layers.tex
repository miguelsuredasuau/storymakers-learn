...
\sourcefig{infographics/ch05-matryoshka.jpg}{Blocks, Loops, and Slides nest like Matryoshka dolls: each layer only works if the one containing it is sound. Fix the outer doll first.}
...
Not every argument unfolds the same way. The Storymakers Method identifies five Loop patterns that cover the vast majority of business argumentation:
...
\sourcefig{infographics/ch05-headline-test.jpg}{The Headline Test: read only the titles in sequence. Do they tell the complete story? Topic titles fail. Action titles pass.}
...
The Headline Test requires action titles, not topic titles (see Chapter 6 for the full comparison and worked examples). ``Market Overview'' fails. ``European demand recovered to 94\% of pre-COVID levels'' passes. If you can read only the titles in sequence and follow the complete argument, the structure works.
